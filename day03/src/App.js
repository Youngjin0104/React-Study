...
function App() {
  ...
  const onChange = (e) => {
    const { name, value } = e.target; // onChange가 일어난 input의 name,value 속성
    setInputs({
      ...inputs,    // 기존의 input 태그 상태 즉 공백
      [name]: value // 이벤트가 일어난 name속성의 input 태그의 입력된 값(value)
    });
  }
  ...
  const onCreate = () => {
    ...
    setInputs({   // 배열에 추가 후 입력창들에 빈 공백으로 초기값 설정
      username: '',
      email: ''
    });

    nextId.current += 1; // 다음에 추가될 user의 인덱스 값을 미리 연산
  }

  // 리스트삭제함수
  const onRemove = (id) => {
    setUsers(users.filter(user => user.id !== id));
    // filter - 조건에 해당되는 값만 추출
  }

  // 사용자를 선택했을 때 변화를 주는 함수(조건부 랜더링)
  const onToggle = (id) => {
    setUsers(
